...
- 가능하면(?) `static` 메서드보다 생성자에서 `validation`을 하도록 하자(이유는 아직 정확하게 이해하지 못했다..). 하지만 그렇다고 `static` 메서드에서 유효성 검증을 하면 안된다는 것은 아니다. 필요하면 얼마든지 할 수 있다.
- 원시값을 포장하는 경우, `VO`일 확률이 높다.
...
- `Money`의 `add`, `multiply`, `divide` 등의 연산 기능을 수행했을 때, `BigDecimal`로 결과값을 리턴하게 되면 해당 값에 대한 유효성 검증을 할 수 없게 된다. 따라서 `Money`의 연산 기능들의 리턴 타입은 `Money`여야 하지 않을까?
  - [간단한 참고자료, 예시](https://github.com/woowacourse/java-lotto/blob/rok93/src/main/java/domain/Money.java)
